# Azure Disk Storage: Premium SSD v2 が US Gov Arizona リージョンで一般提供開始

**リリース日**: 2026-03-31

**サービス**: Azure Disk Storage

**機能**: Premium SSD v2 Disk の US Gov Arizona リージョン対応

**ステータス**: Launched (GA)

[このアップデートのインフォグラフィックを見る](https://takech9203.github.io/azure-news-summary/20260331-premium-ssd-v2-us-gov-arizona.html)

## 概要

Azure Premium SSD v2 が US Gov Arizona リージョンで一般提供 (GA) を開始した。US Gov Arizona は Availability Zone (AZ) を持たないリージョンであり、Azure Government クラウドにおいて Premium SSD v2 が利用可能となる重要なアップデートである。
...
- US Gov Arizona リージョンのユーザーは Premium SSD v2 を利用するために US Gov Virginia など他の Government リージョンにデプロイする必要があり、レイテンシが増加する場合があった
- 米国政府機関向けのデータレジデンシー・コンプライアンス要件を満たしつつ高性能ストレージを利用するリージョン選択肢が限定されていた
- Premium SSD v1 では IOPS やスループットがディスクサイズに固定されており、過剰プロビジョニングによるコスト増が発生していた

**アップデート後の改善**

- US Gov Arizona リージョンで Premium SSD v2 をローカルにデプロイ可能となり、Government クラウドにおけるストレージ選択肢が拡大された
- サブミリ秒のディスクレイテンシにより、政府機関のデータベースやトランザクション処理ワークロードの応答性が向上する
...
flowchart TD
    ...
    subgraph GovRegions["Azure Government の Premium SSD v2 対応"]
        GVA["US Gov Virginia<br/>3 AZ 対応"]
        GAZ["US Gov Arizona<br/>AZ なし - NEW"]
    end

    PSDV2 --> GovRegions

    style PSDV2 fill:#0078d4,color:#fff
    style GAZ fill:#e3008c,color:#fff
    style GVA fill:#0078d4,color:#fff
```

この図は、Azure VM のストレージアーキテクチャにおける Premium SSD v2 の位置付けと、Azure Government リージョンでの利用可能リージョンを示している。Premium SSD v2 はデータディスクとしてのみ利用可能であり、OS ディスクとしては使用できない。US Gov Arizona は AZ なしのリージョンとして新たに対応が追加された。

## サービスアップデートの詳細

### 主要機能

1. **US Gov Arizona リージョンでの一般提供**
   - Availability Zone を持たない Government リージョンでの Premium SSD v2 提供
   - AZ なしリージョンではインフラストラクチャ冗長性なし (No infrastructure redundancy required) でデプロイ
   - Azure Government クラウドにおける Premium SSD v2 対応リージョンが US Gov Virginia に加えて 2 リージョン目となる
...
US Gov Arizona のような AZ なしリージョンでは、`--zone` パラメータを指定せずにデプロイする。
...
- **Government コンプライアンス対応**: US Gov Arizona リージョンでのローカルデプロイにより、米国政府機関のデータレジデンシーおよびコンプライアンス要件に対応可能
- **Government クラウドの冗長性向上**: US Gov Virginia に加え US Gov Arizona でも利用可能となり、Government クラウド内での地理的分散が可能に
...
- **AZ なしリージョンでのレイテンシ**: US Gov Arizona は AZ を持たないリージョンであるため、AZ 対応リージョンと比較して平均レイテンシがわずかに高くなる可能性がある
...
- Azure Government の料金は商用クラウドと異なる場合がある。詳細は [Azure Government の料金ページ](https://azure.microsoft.com/pricing/details/managed-disks/) を参照

## 利用可能リージョン

Premium SSD v2 は多数のリージョンで利用可能である (2026 年 3 月時点)。

### Azure Government リージョン

| リージョン | AZ 対応 |
|---|---|
| US Gov Virginia | 3 AZ 対応 |
| **US Gov Arizona (今回追加)** | **AZ なし** |

### 商用クラウドの AZ なしリージョン (抜粋)

Australia Central 2、Australia Southeast、Brazil Southeast、Canada East、Germany North、India South、North Central US、Norway West、Switzerland West、Taiwan North、UK West、West Central US、West US

### 商用クラウドの 3 AZ 対応リージョン (抜粋)

Austria East、Australia East、Brazil South、Canada Central、Central India、Central US、East Asia、East US、East US 2、France Central、Germany West Central、Japan East、Korea Central、North Europe、South Central US、Southeast Asia、Sweden Central、Switzerland North、UK South、West Europe、West US 2、West US 3
...
- [インフォグラフィック](https://takech9203.github.io/azure-news-summary/20260331-premium-ssd-v2-us-gov-arizona.html)
- [公式アップデート情報](https://azure.microsoft.com/updates?id=559677)
...
Azure Premium SSD v2 が US Gov Arizona リージョンで一般提供を開始した。US Gov Arizona は Availability Zone を持たない Azure Government リージョンであるが、Premium SSD v2 の利用が可能となり、米国政府機関のユーザーがサブミリ秒レイテンシの高性能ブロックストレージを US Gov Virginia 以外の Government リージョンでも利用できるようになった。これにより、Government クラウド内での地理的分散やディザスタリカバリの選択肢が拡大される。Premium SSD v2 は容量・IOPS・スループットを個別に設定できる柔軟な課金モデルにより、Premium SSD v1 と比較して多くのワークロードでコスト効率が向上する。最大 80,000 IOPS、最大 1,200 MB/s のスループットを提供し、データベース、トランザクション処理など幅広いワークロードに適用可能である。ただし、OS ディスクとしては使用できない点、ホストキャッシュに非対応である点、AZ なしリージョンでは平均レイテンシがわずかに高くなる可能性がある点には留意が必要である。

---

**タグ**: #Azure #DiskStorage #PremiumSSDv2 #USGovArizona #AzureGovernment #GA #Storage #Compute #VirtualMachines #BlockStorage
